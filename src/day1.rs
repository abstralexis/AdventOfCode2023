...
pub fn solution() -> i32 {
    dbg!(numeric_insertions(include_str!("day1.txt")))
    .lines()
    .map(|line| {
        &line;
        format!("{}{}",
            line.chars().nth(line.find(char::is_numeric).unwrap()).unwrap(), 
            line.chars().nth(line.rfind(char::is_numeric).unwrap()).unwrap()
        )
    })
    .fold(0, |acc, strnum| {
        acc + strnum.parse::<i32>().unwrap()
    })
}

pub fn numeric_insertions<'a>(s: &'a str) -> String {
    ...
    for (word, num) in map.iter() {
        let replacer = dbg!(format!("{word}{num}{word}"));
